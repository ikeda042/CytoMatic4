...
def calculate_cell_length(db_name: str):
    ...
    with Session() as session:
        ...
        n_1, n_2 = -1, -1
        image_for_size = cv2.imdecode(
            np.frombuffer(cells[0].img_ph, dtype=np.uint8), cv2.IMREAD_COLOR
        )
        ...
        for cell in tqdm(cells):
            if cell.manual_label == 1:
                ...
                if eigenvalues[1] < eigenvalues[0]:
                    ...
                    U = [
                        Q.transpose() @ np.array([i, j])
                        for i, j in coords_inside_cell_1
                    ]
                    U = [[j, i] for i, j in U]
