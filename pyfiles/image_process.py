...
from .unify_images import unify_images_ndarray
...
def image_process(input_filename: str = "data.tif",
                    ...
                    param2: int = 255,
                    image_size:int = 100,
                    draw_scale_bar: bool = True) -> None:
    engine = create_engine(f'sqlite:///{input_filename.split(".")[0]}.db', echo=True)
    Base.metadata.create_all(engine)
    Session = sessionmaker(bind=engine)
    num_tif = init(input_filename=input_filename, param1=param1, param2=param2,image_size=image_size)
    for k in tqdm(range(0,num_tif//3)):
        for j in range(len(os.listdir(f'TempData/frames/tiff_{k}/Cells/ph/'))):
            ...
            if contours != []:
                ...
                unify_images_ndarray(image1=image_ph_copy, image2=image_fluo1_copy, image3=image_fluo2_copy ,output_name=f"TempData/frames/tiff_{k}/Cells/unified_images/{j}")
                unify_images_ndarray(image1=image_ph_copy, image2=image_fluo1_copy, image3=image_fluo2_copy ,output_name=f"TempData/app_data/{cell_id}")
